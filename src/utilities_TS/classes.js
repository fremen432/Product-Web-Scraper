	...
	async OPERATION_getProductInfo() {
		return await this.page.evaluate(() => {
			...
			const selector_productImage =
				"div.s-product-image-container.aok-relative.s-image-overlay-grey.s-text-center.s-padding-left-small.s-padding-right-small.s-flex-expand-height > div > span > a > div > img.s-image";
			...
			for (el of resObj) {
				...
				const image = el.querySelector(selector_productImage)
					? el.querySelector(selector_productImage).src
			...
			return resultArray.filter(
				(el) => el.ProductName != "No product name Found"
			);
		});
	}
...
export class EbaySearch extends Search {
	...
	async OPERATION_getProductInfo() {
		return await this.page.evaluate(() => {
			...
			const selector_productImage =
				"div.s-item__image-section > div > a > div > img";
			...
			for (el of resObj) {
				...
				const productImage = el.querySelector(selector_productImage)
					? el.querySelector(selector_productImage).src
					: "No product image found";
...
export class CraigslistSearch extends Search {
	constructor(inputs) {
		...
		*/
		...
		// https://craigslist.org/search/sss?query=xbox
	}
	async OPERATION_getProductInfo() {
		return await this.page.evaluate(() => {
			const resultArray = [];
